...
void	PPU::Reset()
{
	...
	if( lpScreen )
		::memset( lpScreen, 0x3F, SCREEN_WIDTH*SCREEN_HEIGHT*sizeof(BYTE) );	//0x3F(RGB 색상으로 거의 검정색, ascii코드로는 물음표)로 셋팅
}
...
//vblank는 영상표시장치에서 다음 스크린 필드를 찾기 위해 스크린의 첫번째 라인으로 들어가는 동안 잠시 멈추는 기간
void	PPU::VBlankStart()
{
	PPUREG[2] |= PPU_VBLANK_FLAG;	//vblank 셋팅
}

void	PPU::VBlankEnd()
{
	PPUREG[2] &= ~PPU_VBLANK_FLAG;	//vblank 해제
	// VBlank 탈출할 때까지 제거
	// 익사이트 바이크에서 중요
